...
export const UNIT_STEPS_CONFIG = [
  {
    ...
    subtitle: 'Pre-test (ชุดข้อสอบคงที่)',
    icon: 'FileQuestion',
  },
  {
...
export function setUnitStepCompleted(
  ...
  if (step === 'pretest') {
    ...
    updated.lesson_done = true; // finishing quiz implicitly implies lesson completed
...
export function isStepUnlocked(input: string | null | undefined, step: UnitStepKey): boolean {
  ...
  switch (step) {
    case 'pretest':
      ...
      return true;
    case 'lesson':
      // Step 2 unlocked only after pre-test is done
      return progress.pretest_done;
    case 'unit_quiz':
      // Step 3 unlocked only after lesson is done (or pre-test done and student studied lesson)
      return progress.pretest_done && progress.lesson_done;
    case 'game':
      // Step 4 unlocked only after unit quiz is passed
      return progress.unit_quiz_done;
    case 'quest':
      // Step 5 unlocked only after game is finished
      return progress.game_done;
    case 'posttest':
      // Step 6 unlocked only after quest is completed
...
export function getStepUrl(input: string | null | undefined, step: UnitStepKey): string {
  ...
  switch (step) {
    case 'pretest':
      return `/student/assessment?unit=${unitId}&subdomain=${subDomain}&type=pre_test`;
    case 'lesson':
    ...
    case 'posttest':
      return `/student/assessment?unit=${unitId}&subdomain=${subDomain}&type=post_test`;
    default:
      return `/student/lessons/${unitId}`;
  }
}
